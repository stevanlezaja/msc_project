...
class DifferentialEvolutionController(Controller):
    ...
    def get_control(self, curr_input: ra.RamanInputs, curr_output: ra.Spectrum[ct.Power], target_output: ra.Spectrum[ct.Power]) -> ra.RamanInputs:
        ...
        result = scipy.optimize.differential_evolution(optimization_fn, bounds=[[0.25, 0.75], [1420, 1490]], maxiter=1000)
        return ra.RamanInputs(powers=[ct.Power(float(result.x[0]), 'W')], wavelengths=[ct.Length(float(result.x[1]), 'nm')]) - curr_input
